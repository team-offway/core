...
- **실측(2026-08-28 · 240콜) — `areaBasedList2` 응답시간 분포.** 6초 타임아웃(#238)의 근거를 찾으려고
  **개발 PC 와 운영 서버에서 각각** 쟀다. 다시 잴 수 있게 둘 다 레포에 있다 —
  로컬은 `scripts/measure_tour_latency.py`, 서버는 `Diagnose` 워크플로의 `tour_latency_sample` 스위치.
  대상 지역은 스크립트가 단독으로 소유하고 워크플로가 그것을 불러 쓴다.

  앱과 같은 요청이다 — `arrange=B`, `numOfRows=100`, `contentTypeId` 는 없음/39/32
  (`TourApiClientImpl.findByArea` + `RegionPoiService.CANDIDATE_ROWS`).
  표본은 콘텐츠가 많은 곳과 적은 곳을 섞은 **10지역 × 3종류 × 4회**, 종류를 번갈아 불러 시간 편향을 줄였다.
  지역은 (시도, 시군구)로 든다 — 고성군이 강원(32:2)·경남(36:3) 둘이라 이름만으로는 재현이 안 된다.
  여기 쓴 것은 **경남 고성군**이다.

  | 종류 | n | p50 | p95 | p99 | 최댓값 | 평균 응답 | 실패 | 6초 초과 |
  |---|---|---|---|---|---|---|---|---|
  | **개발 PC** 전체타입 | 40 | 316ms | 2,287ms | 2,298ms | 2,298ms | 47.5KB | 0 | **0** |
  | **개발 PC** 맛집(39) | 40 | 275ms | 458ms | 2,287ms | 2,287ms | 19.8KB | 0 | **0** |
  | **개발 PC** 숙박(32) | 40 | 167ms | 218ms | 2,183ms | 2,183ms | 4.8KB | 0 | **0** |
  | **EC2(도쿄)** 전체타입 | 40 | 444ms | 485ms | 507ms | 507ms | 47.5KB | 0 | **0** |
  | **EC2(도쿄)** 맛집(39) | 40 | 396ms | 546ms | 2,451ms | 2,451ms | 19.8KB | 0 | **0** |
  | **EC2(도쿄)** 숙박(32) | 40 | 267ms | 329ms | 340ms | 340ms | 4.8KB | 0 | **0** |

  **#238 이 세운 "전체타입 조회만 유독 느린가" 가설은 두 표본 어디서도 확인되지 않는다.** 응답이 10배
  무거운 전체타입(47.5KB)이 서버에서 오히려 꼬리가 가장 짧다(507ms). 그러니 **타입을 나눠 부르는 것은
  이 표본들이 뒷받침하지 않는다.**

  p50 은 응답 크기를 따라간다(47.5KB→444ms, 4.8KB→267ms). 반면 꼬리는 크기와 무관하게 튄다 —
  서버에서 유일하게 2초를 넘긴 것은 가장 무거운 조회가 아니라 맛집(39) 한 건이었다.

  > **"같은 순간에 함께 느려진다" 고는 말할 수 없다.** 두 측정 모두 호출을 **순차로** 보내고 호출 시각을
  > 기록하지 않는다. 서로 다른 시각에 찍힌 최댓값이 비슷하다는 것만으로 동시성을 주장할 수 없다.
  > (이 문서의 앞선 판(2026-08-28)에 그 주장이 있었고, 서버 측정이 곧바로 반증했다 — 로컬에서 2.2~2.3초로
  > 뭉쳐 보이던 최댓값이 서버에서는 507·2,451·340ms 로 흩어졌다.)

  **이 표본들로도 상수를 정하지 않는다.** 남은 한계가 그대로다.
  - 종류당 n=40 이라 **p99 가 사실상 최댓값**이다. 규약이 "표본 최대값은 쓰지 않는다" 고 한 그 자리다
  - 각 **한 시간대 1회 측정**이다. #238 의 6,676ms 는 다른 날 13:41 이고 둘 다에서 재현되지 않았다
  - 240콜 중 **6초를 넘긴 것이 0건**이다. 정상 경로를 재는 데는 충분하지만, **꼬리를 재려면 드물게
    일어나는 사건을 잡아야 한다** — 이 표본 크기로는 못 잡는다

  다만 두 측정이 함께 말해 주는 것이 하나 있다. **6초는 정상 경로에서 아주 멀다** — 서버 p95 가 종류를
  통틀어 546ms 이하이므로 상한이 그 11배다. 재시도를 같은 벽시계 예산 안에 넣을 여지가 그만큼 있다.
...
- **발급**: data.go.kr 데이터셋 **15128559** `한국관광공사_기초지자체 중심 관광지 정보` → [활용신청](https://www.data.go.kr/data/15128559/openapi.do). 인증키는 공통이지만 **활용신청은 1-3(15101972)과 별개**다.
- **한도**: 개발계정 **1,000/일** (포털 확인). ⚠️ **1-3 과 나눠 쓰지 않는다** — 예전에 이 문서가 "같은 계정 → 별도 신청 없이" 라고 적어 둔 탓에 코드가 셋을 한 한도로 합쳐 셌고, 남의 소비가 이쪽 한도를 먹어 배치가 멈췄다(#496). **키가 같은 것과 한도가 같은 것은 다르다.**
...
### 1-3-1-1. 관광지별 연관 관광지 — 한국관광공사 · **실호출 확인(#186)**

중심 관광지(1-3-1)와 **실제로 함께 가는 곳**을 순위로 준다. 좌표 군집이 답하지 못하는 "왜 이 조합인가" 가 여기 있다.

- **발급**: data.go.kr 데이터셋 **15128560** `한국관광공사_관광지별 연관 관광지 정보` → [활용신청](https://www.data.go.kr/data/15128560/openapi.do). **1-3·1-3-1 과 각각 별개 활용신청**이고, 한도도 따로다(개발계정 **1,000/일**, 포털 확인).
- **엔드포인트**: `https://apis.data.go.kr/B551011/TarRlteTarService1/areaBasedList1`
  - params: `serviceKey` · `MobileOS` · `MobileApp` · `_type=json` · `areaCd`(시도 2자리) · `signguCd`(법정동 5자리) · `baseYm` · `numOfRows` · **`pageNo`(필수)**
- **응답 필드**: `tAtsCd`·`tAtsNm`(중심) · `rlteTatsCd`·`rlteTatsNm`(연관) · `rlteRank` · `rlteCtgryLclsNm`(관광지·음식·숙박) · `rlteCtgryMclsNm` · `rlteSignguNm`
- **함정 — 1-3-1 과 같은 계열이라 그대로 물려받는다**
  - **법정동 코드를 쓴다.** TourAPI 코드를 넣으면 `resultCode=0000` 에 0건이 와서 **조용히 빈다**
  - **`pageNo` 가 필수.** 빠뜨리면 `resultCode=11` 이 `response` 래퍼 **없이** 최상위로 온다
  - 결과 없으면 `items` 가 **빈 문자열**, 1건이면 `item` 이 배열이 아니라 **단일 객체**
- **⚠️ 좌표를 주지 않는다.** 응답에 `mapX`·`mapY` 가 없다(중심관광지 API 는 준다). 그래서 **인허가 장소(#144)와 이름으로 이어** 좌표를 얻고, 못 이으면 담지 않는다.
  - 이름 표기가 제각각이라 정규화가 필요하다 — `동해원/[중식]` → `동해원`, `유천냉면/공주점` → `유천냉면`
  - 실측(공주시): 연관 음식 **75곳 중 54곳(72%)** 매칭
- **⚠️ 지역 밖이 섞인다.** 실측(공주시) 300건 중 **45건(15%)** 이 천안·아산·부여였다(온양온천역·롯데아울렛 부여점 등). `rlteSignguNm` 으로 거르며, 그 필터는 **어댑터가 강제**한다
- **비용**: 지역당 1콜 × 89곳 = **월 89콜**(이 서비스 자체 한도 1,000 의 8.9%). 하루 한 번 가드가 재배포 중복 호출을 막는다 — 지역별 기준월 마커만으로는 원본 미발행 기간에 안 막혔다(#495)
- **OffWay 용도**: 코스 조립의 볼거리·끼니 순서(#186). 연관 데이터가 없는 지역은 좌표 군집으로 degrade

### 1-3-2. 관광사진 갤러리 — 한국관광공사 · **실호출 확인(#196)**
- **발급**: 인증키는 공통이지만 **활용신청은 별개**다. ⚠️ 한도를 1-3 과 나눠 쓴다고 읽으면 안 된다 — 이 문서의 옛 문장이 그 오해를 만들었다(#496).
...
### 1-3-3. 전국문화축제표준데이터 — 공공데이터포털 · **활용신청 필요** (#433)

지자체 **229곳**이 같은 스키마로 올린 것을 포털이 **매월 초 병합**해 전국 단위로 낸다. 지역별 어댑터 89개를 만들 필요가 없다.

- **왜 붙였나**: TourAPI `searchFestival2` 를 89곳 전수로 돌린 결과가 **합계 1건**이었다(#392). 지역이 얇은 게 아니라 **축제 타입만 비어 있다** — 안동국제탈춤페스티벌이 있는 안동시가 0인데 같은 지역이 볼거리·맛집·숙박은 임계를 넉넉히 넘긴다.
- **데이터 ID**: `15013104` · 개발·운영 **자동승인** · 개발계정 **10,000/일**
- **한도가 TourAPI 와 별개다.** 관광 한도가 마른 날에도 이 적재는 돈다.
- **엔드포인트**: `https://api.data.go.kr/openapi/tn_pubr_public_cltur_fstvl_api`
  - params: `serviceKey` · `type=json` · `pageNo`(1부터) · `numOfRows`
- **필드**: 축제명 · 개최장소 · 개최시작일 · 종료일 · 축제내용 · 주관/주최/후원기관 · 전화번호 · 홈페이지 · 도로명·지번주소 · **위도 · 경도**
- **⚠️ 응답 필드의 영문명을 아직 실호출로 확정하지 못했다.** 포털 상세 페이지가 한글 항목명만 표로 보여주고 영문 명세를 공개하지 않는다. 어댑터는 표준데이터 계열의 통상 표기(`fstvlNm`·`fstvlStartDate`·`rdnmadr`·`latitude` 등)를 상수 한 곳에 모아 두고, **행은 왔는데 한 건도 못 읽으면 warn + 실제 키 목록을 남긴다** — 필드명이 틀렸을 때 "축제 0건" 이 정상처럼 보이지 않게. 첫 실호출로 확정하고 이 문서를 고친다.
- **실측(2026-09-04, 포털 그리드 전량 다운로드 1,305건 기준)**
  | | TourAPI | 표준데이터 |
  |---|---|---|
  | 89곳 합계 | 1건 | **446건** |
  | 좌표 있는 것 | 1건 | **345건** |
  | 커버리지 | 3 / 89곳 | **88 / 89곳** |
  | 지역당 최소·중앙·최대 | 0 · 0 · 1 | 0 · **3** · 26 |
- **함정**
  - **사진이 없다.** TourAPI 와 달리 카드에 쓸 이미지가 안 온다.
  - **좌표 없는 것이 446 중 101건.** 지자체마다 채움률이 다르다(신안군 25건 중 1건, 보령시 26건 전부). 좌표 없으면 동선에 못 올려 후보에서 뺀다.
  - **과거 회차가 섞여 있다.** 2013년 행까지 남아 있고 오늘 이후는 81건이다. 대부분 매년 여는 정기 축제라 그대로 담되, 코스에는 여행일에 열리는 것만 올린다.
  - 응답시간·페이지 수는 **아직 실측 못 했다**(로컬에 키 없음). 첫 배치 로그의 `totalCount` 가 페이지 수를 확정한다.
- **OffWay 용도**: 볼거리 후보(`FST-` 접두사) — 여행일에 열리는 축제를 코스 맨 앞에 올린다.
...
| 2-5 | **여행 지원 혜택** | 전용 API 없음(제안서 명시) | 수동 | **수동 적재**(정책명·기간·대상지·할인) |
...
| 정책 혜택 매칭·뱃지 | 혜택(백오피스) + 인구감소지역 + 로컬100/관광두레 |
...
- **숲나들e**(자연휴양림, foresttrip.go.kr): 농촌체험/치유관광 분류와 매핑되나 **공식 API 없이 스크래핑** → 백엔드 안정성 낮아 보류.
