...
- **실측(2026-08-28 · 120콜) — `areaBasedList2` 응답시간 분포.** 6초 타임아웃(#238)의 근거를 찾으려고 쟀다.
  스크립트는 `scripts/measure_tour_latency.py` 라 같은 조건으로 다시 잴 수 있다.
  ...
  | 종류 | n | p50 | p95 | 최댓값 | 평균 응답 | 실패 | 6초 초과 |
  |---|---|---|---|---|---|---|---|
  | 전체타입 | 40 | 316ms | 2,287ms | 2,298ms | 47.5KB | 0 | **0** |
  | 맛집(39) | 40 | 275ms | 458ms | 2,287ms | 19.8KB | 0 | **0** |
  | 숙박(32) | 40 | 167ms | 218ms | 2,183ms | 4.8KB | 0 | **0** |

  **꼬리가 종류별로 갈리지 않는다.** p50 은 응답 크기를 따라간다(47.5KB→316ms, 4.8KB→167ms).
  그런데 **최댓값은 셋 다 2.2~2.3초로 뭉친다** — 특정 조회가 무거워서가 아니라, 그 순간에 걸린 요청이
  종류와 무관하게 함께 느려진다는 뜻이다. #238 이 세운 "전체타입 조회만 유독 느린가" 가설은 이 표본에서
  성립하지 않으므로, **타입을 나눠 부르는 것은 해법이 아니다.**

  **이 표본으로 상수를 정하지 않는다.** 아래 셋이 남아 있어서다.
  - 종류당 n=40 이라 **p99 가 사실상 최댓값**이다. 규약이 "표본 최대값은 쓰지 않는다" 고 한 그 자리다
  - **한 시간대(오후) 1회 측정**이다. #238 의 6,676ms 는 다른 날 13:41 이고 여기서는 재현되지 않았다
  - **개발 PC 에서 쟀다.** 운영은 EC2(도쿄)라 네트워크 경로가 다르다 — 같은 날 운영에서 장소 상세가
    간헐 502 를 냈는데(재시도하면 200) 이 측정에서는 실패가 0 이었다. 서버 쪽에서 재야 답이 나온다
