...
| ~~1~9일~~ | ~~관광기후지수~~ | `TourStnInfoService1` | **안 쓴다** — 아래 참고 |
...
**관광기후지수** `TourStnInfoService1/getCityTourClmIdx1` — **지금은 쓰지 않는다**(#193)

> 클라이언트·서비스까지 만들었는데 **부르는 화면이 끝내 안 생겼다.** 빈으로 떠 있기만 하고 호출은 0건이라,
> 읽는 사람이 "이건 뭐지" 하게 만드는 코드로 남아 코드를 걷어냈다. **아래 실측은 지운 것이 아니다** —
> 다시 붙일 일이 생기면 이 함정들을 다시 밟을 이유가 없다.
