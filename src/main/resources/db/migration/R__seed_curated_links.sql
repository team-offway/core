...
-- ## 썸네일은 두 곳만 채운다
--
-- 열한 곳의 대표 이미지를 전수로 뒤졌다(2026-09-08). 실제 **사진**이 있는 곳은 넷뿐이고 나머지는
-- 로고·파비콘이거나 아예 없었다 — 국가유산포털은 57x57 파비콘, 코레일은 og:image 의 값이 문자열
-- `"url"` 로 깨져 있고, 고속버스·숲나들e·국립공원은 JS 로 그려 정적 파싱에 안 잡힌다.
--
-- 그래서 **사진이 있고, 이미 들어가 있지 않고, 핫링크가 열려 있는 것**만 넣는다. 지금은 웰촌(750x400)과
-- 숲나들e(300x250) 둘이다.
--
-- 빼는 이유를 남긴다.
--   * 이미 들어간 세 건(구석구석·국가유산포털·코레일) — 이 파일은 기존 행을 건드리지 않는다.
--     `ON DUPLICATE KEY UPDATE id = id` 라 값이 안 바뀐다. 그쪽은 어드민이 올린다
--   * 고캠핑 — 사진(3600x1401)은 있지만 **og:image 경로가 이미 핫링크를 막고 있고**(HTTPError),
--     585KB 를 앱 카드가 매번 받는 것도 과하다. 같은 도메인이라 열려 있는 경로도 언제 막힐지 모른다
--   * 로고·파비콘뿐인 곳 — 카드 자리에 로고를 넣으면 사진 있는 카드와 섞여 오히려 덜 고르다
--
-- **이건 임시값이다.** 기관이 파일을 옮기면 깨지므로, 디자인이 만든 이미지를 어드민(#377 업로드)으로
-- 올리면 그것이 정본이 된다. 여기 값은 그때까지 빈 카드를 줄이는 용도다.
--
...
--
-- **주소가 `letskorail.com` 이 아니다.** 그쪽은 명절 기간에 `bt.korail.com/holiday/main`(명절대수송)
-- 으로 리다이렉트돼, 누르면 예매 화면이 아니라 명절 안내가 뜬다(2026-09-08 확인). `korail.com` 메인도
-- 같이 넘어가는데 하위 경로는 멀쩡해서, 예매 화면으로 바로 가는 주소를 쓴다 — 출발역·도착역·출발일·
-- 인원 폼이 있는 것까지 확인했다.
--
-- **HTTP 200 만 보면 못 잡는 종류다.** 리다이렉트 뒤 페이지가 200 으로 오기 때문이다 — #498 에서
-- 한국관광 100선이 "200 인데 본문은 404" 였던 것과 같은 결이다. 링크는 최종 도착지까지 봐야 한다.
('korail-ticket', '코레일 승차권 예매', '기차표 예매',
 '한국철도공사 공식 예매. 우리가 안내한 열차 시간대로 바로 예매할 수 있다.',
 'https://www.korail.com/ticket/main', NULL,
...
('welchon', '웰촌', '농촌체험 마을',
 ...
 'https://www.welchon.com', 'https://www.welchon.com/_pub/images/sub/main_vr-img07.jpg',
...
('foresttrip', '숲나들e', '자연휴양림 예약',
 ...
 'https://www.foresttrip.go.kr', 'https://www.foresttrip.go.kr/intro/images/intro_forest.jpg',
