...
public interface CourseStorageApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - `UPCOMING` — **종료일**이 오늘 포함 이후, **가까운 것부터**. 여행 중인 코스도 여기 남는다
                    - `PAST` — **종료일**이 오늘보다 앞, 최근 것부터
                    - `ALL`(기본) — 전부, 저장한 순서(최근 저장이 위)

                    **시작일이 아니라 종료일로 가른다**(#325). 시작일 기준이면 2박3일 여행 둘째 날에 그 코스가
                    이미 `PAST` 로 넘어가는데, 앱의 칩과 "다녀오셨나요?" 모달은 종료일 기준이라 셋이 어긋났다.
                    당일치기는 시작일=종료일이라 차이가 없다.
