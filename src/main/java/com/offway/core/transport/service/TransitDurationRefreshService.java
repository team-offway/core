...
public class TransitDurationRefreshService implements ManualBatch {
     ...
     *   30일        1,473          123% ✗           4,420            44%
     *   60일          737           61%             2,211            22%
     *   90일          491           41%             1,473            15%
     * </pre>
     *
     * <p><b>30일은 용량을 넘는다.</b> 전체 44,931 구간 중 미운행이 98.5% 라 재측정 대상이 44,000 건
     * 남짓인데, 30일로 나누면 하루 1,473 구간이다 — 상한을 올리지 않으면 영구히 밀리고, 올리면
     * TAGO 한도의 44% 를 배치가 먹는다. <b>그 키는 열차와 공유해서</b> 거기서 태우면 사용자 코스
     * 생성이 먼저 죽는다.
     *
     ...
     * 90일이 걸리는 것은 "TAGO 가 성공으로 답했고 그날 편이 없다" 뿐이다.
     *
