 ...
 * 서비스가 담당한다. 출발지나 지역에 역이 없으면 열차 자체를 조회하지 않고 {@code NO_STATION} — 오지 인구감소지역엔 흔한 정상 결과다.
 */
@Service
...
public class TrainAccessService {
    ...
    /** 출발 좌표에서 목적지 지역(시도·시군구)까지, 해당 날짜의 열차 접근. */
    public TrainAccess accessTo(double originLat, double originLng, String destSido, String destSigungu, LocalDate date) {
        Optional<Station> from = stationResolver.forOrigin(originLat, originLng);
        Optional<Station> to = stationResolver.forRegion(destSido, destSigungu);
