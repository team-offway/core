...
public class UserWithdrawalService {
     ...
     * 탈퇴시킨다.
     *
     * <p><b>지울 대상은 {@code userId} 하나가 정한다</b>(#280). 코스·연차·후기·알림이 전부 이 키로 묶여
     * 있어, 요청이 무엇을 들고 오든 지워지는 것은 인증으로 확인된 이 사용자의 데이터뿐이다.
     *
     * <p>예전에는 게스트 키를 거쳐야 했고 그 키가 검증 불가능한 헤더에서 왔다 — 남의 값을 적어 보내면
     * 남의 데이터가 지워졌다. 임시로 {@code user_guest_link} 에 기록해 막았지만, 그 기록 자체가
     * 인증 안 된 값으로 만들어져 구멍이 자리를 옮겼을 뿐이었다. 소유가 사용자면 그 구멍이 사라진다.
     *
     * <p>순서가 정해져 있다.
     *
     * <ol>
