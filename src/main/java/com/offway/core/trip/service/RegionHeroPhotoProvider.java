...
public class RegionHeroPhotoProvider {
     ...
     *
     * <p>교통·숙박·상업·레저시설은 "그 지역에 왜 가는가" 에 답하지 못한다. 실측에서 이 규칙이 바꾼 곳:
     * 부산 동구(부산역 → 부산항), 단양군(단양구경시장 → 만천하스카이워크), 평창군(휘닉스 파크 →
     * 대관령양떼목장).
     *
     * <p><b>골프클럽은 {@code CC} 와 {@code GC} 둘 다,</b> 이름 끝이 아니어도 막는다. 처음엔 {@code CC$}
     * 만 봤는데 89곳 전수에서 {@code GC} 로 끝나는 골프장이 13곳(가평베네스트GC·문경GC·알펜시아700GC 등),
     * {@code CC} 가 중간에 오는 것도 있었다(포웰CC프린세스).
     *
     * <p><b>스키 리조트는 브랜드명이라 업태로 안 잡힌다.</b> "휘닉스 파크" 는 대분류가 관광지, 중분류가
     * 문화관광이고 이름에 "리조트"·"스키장" 이 없어 다른 규칙에 전부 걸리지 않는다. 89곳 상위 20위를 훑어
     * 실제로 등장하는 브랜드만 담았다 — 목록을 늘리는 대신 새로 뽑히는 것이 보이면 그때 추가한다.
     */
    private static final Pattern NOT_HERO = Pattern.compile("골프|컨트리클럽|[CG]C|리조트|콘도|호텔|모텔|펜션|카지노"
            + "|터미널|역$|시장|백화점|아울렛|마트|병원|대학교|스키장|찜질|사우나"
            + "|휘닉스|알펜시아|용평|하이원|비발디|웰리힐리|소노펠리체|오크밸리|엘리시안");
