...
public interface PoiApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    **`images`** — 대표 이미지(`imageUrl`) 말고 **같은 장소의 다른 사진들**(#464). 화면이
                    갤러리로 쓴다. 관광 API 에 등록된 만큼 오고(실측으로 완도타워가 16장) 최대 10장까지
                    담는다. 없으면 **빈 배열**이다 — `null` 이 아니라 배열이라 매번 검사하지 않아도 된다.
                    우리 DB 에서 온 장소는 항상 빈 배열이다.
