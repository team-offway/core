...
public interface LeaveUsageRepository {
    ...
    List<LeaveUsage> findByUserId(UUID userId);

    /** 증감 합 — 쓴 연차. 내역이 없으면 0. */
    double sumDaysByUserId(UUID userId);
     ...
     *
     * <p>{@link #existsByUserIdAndCourseId} 와 달리 <b>행 자체</b>가 필요하다. 재계산의 입력인 반차 여부가
     * 그 행에 있고, 갱신도 그 행에 한다.
     *
     * <p>{@code uk_leave_usage_user_course} 가 코스당 한 행을 강제하므로 결과는 하나 이하다.
     */
     ...
     *
     * <p>코스마다 {@link #existsByUserIdAndCourseId} 를 부르면 N+1 이라 한 번에 모아온다.
     */
    Set<Long> findDeductedCourseIds(UUID userId);

    /**
     * 코스 차감 내역을 지운다(차감 취소).
     *
     * <p>음수 행을 덧붙이지 않는 이유 — {@code uk_leave_usage_user_course} 가 코스당 한 행을 강제한다(#91).
