...
public interface LeaveApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    **사유(reason)와 상세 메모(memo)는 다른 칸이다**(#319). 사유는 한 줄 라벨, 메모는 풀어 쓰는
                    자리다. 한쪽이 다른 쪽을 덮지 않으므로 둘 다 보내도 된다. 길이를 넘기면 400 이 아니라
                    잘라서 저장한다 — 부가 정보라 요청을 되돌릴 만큼은 아니다.

                    **여러 날을 한 번에 등록하는 계약은 없다.** 날짜마다 이 요청을 따로 보낸다. 연속하지 않은
                    날짜(월·수 반차 등)도 같은 방식이다. 한 요청에 여러 날을 싣는 형태를 두지 않은 이유는,
                    그 계약이 생기면 부분 실패를 어떻게 답할지(무엇이 저장됐는지)를 함께 정해야 하는데
                    지금은 그 필요가 확인되지 않았기 때문이다. 필요해지면 그때 연다.
    ...
    @Operation(
            ...
            description = """
                    ...
                    **사유·상세 메모를 지우려면 빈 문자열(`""`)을 보낸다.** 필드를 빼거나 null 을 보내는 것은 "그대로 두라" 는
