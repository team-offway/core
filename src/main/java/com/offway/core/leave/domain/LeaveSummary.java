...
public record LeaveSummary(double totalDays, double usedDays, double ledgerDays) {
    ...
    /**
     * 남은 연차가 상한을 넘는가 — 화면이 약속한 입력 범위를 넘는 총 연차가 들어왔다는 뜻이다(#558).
     *
     * <p><b>이 판정을 여기 두는 이유</b>는 상한이 잔여의 성질이고, 잔여를 파생하는 곳이 여기뿐이기
     * 때문이다. 서비스가 {@code 총 - 사용 합} 을 직접 계산해 재면 같은 뺄셈이 두 곳에 생기고,
     * "잔여는 총을 넘지 않는다" 는 이 값객체의 기존 불변식과 갈라져 관리된다.
     *
     * <p>조회 경로는 이걸 묻지 않는다. 이미 저장된 상태를 막아도 사용자가 할 수 있는 게 없고, 사용
     * 내역을 지워 잔여가 늘어난 것은 <b>연차를 안 썼다고 정정한 것</b>이라 정상이다. 묻는 곳은 총 연차를
     * 새로 받는 자리 하나다.
     */
    public boolean exceedsRemainingCap() {
        return !LeaveDays.isValidRemaining(remainingDays());
    }
