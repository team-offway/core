...
public interface DeviceApi {
    ...
    @Operation(
            ...
            description =
                    ...
                    **같은 토큰을 여러 번 보내도 행이 늘지 않는다.** 토큰이 기기의 신원이라, 이미 있는 토큰이면
                    소유자·플랫폼·갱신 시각만 고쳐 쓴다. 그래서 **몇 번을 보내도 결과가 같다** — 재시도해도
                    안전하고, 실패했는지 애매하면 그냥 다시 보내면 된다.

                    앱을 지웠다 깔아 게스트 ID 가 새로 발급된 뒤 같은 토큰으로 등록하면 **주인이 새 게스트로
                    바뀐다.** 그 기기를 실제로 쓰는 사람이 새 게스트이기 때문이다.
