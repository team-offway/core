...
/**
 * 여행 지원 혜택 분류 — <b>이 enum 이 분류의 정본이다</b>. 각 상수가 <b>뱃지 문구</b>와 <b>매칭 대상 지역
 * 태그</b>를 보유한다(다형성으로 분기 제거).
 *
 * <p><b>개수를 이름에 박지 않는다.</b> 예전 문구는 "7대 여행 지원 혜택" 이었는데, 그 숫자는 상수를 하나
 * 더하거나 빼는 순간 거짓이 된다. 실제 정책 건수는 백오피스에서 계속 늘고 줄어 더 그렇다 — 지금 몇
 * 개인지 알고 싶으면 문서가 아니라 이 enum 을 본다.
 *
