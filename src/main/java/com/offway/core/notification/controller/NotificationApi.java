...
public interface NotificationApi {
    ...
    @Operation(
            ...
            description =
                    ...
                    **`regionName` 은 그 코스의 지역이다**(#356). 문구가 아니라 **데이터**라 앞의 원칙과
                    어긋나지 않는다 — 앱은 여전히 `type` 으로 문장을 고르고 이 값을 끼워 넣는다
                    (`'정선 여행' 다녀오셨나요?`).

                    문장에 넣기 좋은 짧은 이름이다 — `정선군` 이 아니라 **`정선`**. 다만 **자치구는 접미사를
                    떼지 않는다**(`동구`·`영도구`). 89곳 중 다섯이 자치구인데 한 글자를 떼면 `동구` 가 `동` 이
                    되어 지명이 아니게 된다. 시도는 붙이지 않는다.

                    **코스가 지워졌으면 `null` 이다**(키는 빠지지 않는다). 그때는 지역명 없는 문구로 그리면 된다.
