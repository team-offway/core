    ...
    public List<PushTarget> createForTripsStartingTomorrow(LocalDate today) {
        ...
        for (Course course : departing) {
            if (notificationRepository.saveIfAbsent(Notification.builder()
                    ...
                    .createdAt(now)
                    .build())) {
                created.add(new PushTarget(course.getGuestId(), NotificationType.TRIP_TOMORROW, course.getId()));
            }
        }
        ...
        log.info("여행 전날 알림 생성 travelDate={} 대상={}건 새로 만듦={}건 이미 있음={}건",
                tomorrow, departing.size(), created.size(), departing.size() - created.size());
        return created;
    }
}
