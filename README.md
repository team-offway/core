...
지역 카드의 **대표 사진**은 따로 고른다. 관광공사 **중심 관광지**(실제 이동 데이터 기반 순위) 상위를 훑어 **관광사진 갤러리 6,118건**에서 사진이 있는 첫 명소를 세운다 — 공주시면 공산성이다. 터미널·역·골프장이 1위인 지역이 흔해서 순위를 그대로 쓰지 않는다. **83/89곳**이 이렇게 정해지고, 나머지는 관광 API 사진으로 내려간다.
