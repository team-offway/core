...
| 6 | `deploy.yml` 을 `-p 127.0.0.1:8080:8080` 으로 | PR | 443 으로만 |
...
지금은 `tls` 에 적은 주소로 Let's Encrypt 가 만료 임박 메일을 보낸다. 그것을 놓치지 않는 것이
1차 방어다. 배포 스모크에 인증서 남은 기간 확인을 붙이는 것은 6단계에서 함께 정한다 —
`deploy.yml` 이 이미 그 도메인을 부르게 되기 때문이다.

수동 확인은 위 `openssl` 한 줄이면 된다.
