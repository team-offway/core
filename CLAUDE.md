...
**모든 작업이 아래 셋에 답해야 한다.** 기능이 되는 것과 그 기능이 우리 제약 안에서 사는 것은 다른 문제다. 답이 "모르겠다" 면 재고 나서 답한다 — 셋 다 실측할 수 있는 질문이다.

### 1. 운영에서 버티는가 — EC2 도커 안의 MySQL 하나가 전부다

운영 DB 는 EC2 인스턴스 위 도커 컨테이너의 MySQL 이다. 관리형 DB 가 아니고 스케일아웃도 없다. **부팅 시 적재·인덱스·테이블 크기가 늘어나는 변경은 그 인스턴스 하나가 감당할 수 있는지 먼저 잰다.**

- **적재를 늘리면 부팅 시간을 잰다.** 빈 DB 기준 최초 부팅과, 파일이 그대로일 때의 재부팅을 **둘 다** 잰다. 후자가 실제 배포마다 치르는 값이다.
- **체크섬·마커로 건너뛰게 만든다.** 재배포마다 전량 재적재하면 배포가 그만큼 느려지고, 그 사이 서버는 옛 데이터로 응답한다.
- **테이블 크기와 컨테이너 메모리를 함께 본다.** 행 수만 보면 인덱스가 붙었을 때를 놓친다.

실측 예(2026-08-11, 로컬 도커 MySQL 8.4): 최초 부팅 3.79초(장소 풀 121,393건 15.9초 + 국가유산 3,437건 0.7초), **재부팅 2.64초**(체크섬 적중으로 둘 다 건너뜀), DB 51MB, 컨테이너 514MB.

### 2. 외부 API 한도를 우리가 갉아먹고 있지 않은가

TourAPI·TMAP 은 **일일 한도가 있고 키를 하나로 공유한다.** 한도가 마르면 사용자 요청이 먼저 죽는다. 그런데 그 한도를 태우는 것이 사용자가 아니라 **우리 쪽 작업일 때가 많다.**

- **배포·부팅이 소비하는 양을 센다.** `@Scheduled(fixedDelay)` 는 프로세스가 살아 있는 동안의 간격이라, 재배포하면 주기가 처음부터 다시 센다 — "주 1회" 라고 적어 두고 배포마다 도는 일이 실제로 있었다(#226·#231).
- **조사·실측도 한도를 쓴다.** 응답 필드 채움률을 재느라 500건 넘게 쓰고 그날 운영 코스 생성을 degrade 시킨 적이 있다(2026-08-11). 실측 전에 남은 한도를 가늠하고, 표본 수를 먼저 정한다.
- **그리고 쓴 콜을 집계에 남기고 디스코드로 알린다** — `scripts/external/probe-notify.sh`. 실측은 앱을 거치지 않아 `ExternalApiCallRecorder` 가 모르는데, 그러면 둘이 동시에 틀어진다: **앱은 한도가 남았다고 판단해 배치를 발사하고**, 10% 단계마다 울리는 한도 알림(#257·#285)도 안 뜬다. 2026-08-11 이 조용했던 이유가 정확히 그것이다 — 아무 기록도 안 남는 소비였다. #590 에서 열차 간선 여부를 재느라 1,073콜을 쓰고도 같은 일이 반복돼 도구로 만들었다.
- **오탐을 먼저 의심한다.** 표본 몇 개를 알려진 정답과 대조하지 않고 전량 측정 결과를 그대로 쓰면, 틀린 필터가 멀쩡한 데이터를 지운다. #590 의 1차 측정은 기준 허브를 간선 종착지 넷으로만 잡아 **정선역·왕십리역을 미운행으로 판정**했다(지선이라 그 넷에 안 닿는다). 그대로 걸렀으면 목록에서 사라졌을 역이다.
- **한도를 안 쓰는 대체 출처를 먼저 찾는다.** 국가유산청 API 는 인증키가 필요 없고 TourAPI 한도와 무관하다 — 같은 목적을 그쪽으로 옮길 수 있으면 그게 낫다.
- 새 외부 호출을 붙일 땐 **"이 화면 한 번에 몇 건 나가나" 를 숫자로 답한다.** 슬롯마다 부르는 설계는 코스 하나에 20건이 넘는다.

### 3. 코스의 완성도가 실제로 올라가는가

우리가 만드는 것은 **여행 코스**다. 어떤 변경이든 마지막 질문은 "그래서 코스가 나아지는가" 다.

- **후보를 늘렸으면 무엇이 늘었는지 본다.** 건수가 아니라 **쓸 수 있는 건수**다 — 좌표 없는 장소는 동선에 못 올리고, 소장 유물은 목적지가 될 수 없다.
- **카드가 채워지는지 본다.** 이름·주소만 있는 후보는 코스에 들어가도 화면이 빈다. 사진·소개·운영시간이 함께 오는 출처가 더 값어치 있다.
- **지역 편차를 본다.** 전체 평균이 올라도 얇은 지역이 그대로면 그 지역 사용자에게는 아무 일도 안 일어난 것이다. 89곳 커버리지와 지역당 최소·중앙값을 함께 낸다.
- 답이 "코스와 무관한 내부 개선" 이면 그렇게 적는다. 그건 정당한 답이지 회피가 아니다.
...
- 워밍 주기는 `TTL − 워밍 1회 소요시간 − 장애 여유` 로 잡는다. **"TTL 보다 짧게" 만으로는 만료 공백이 안 막힌다** — 스케줄러 지연·워밍 실패·외부 지연이 겹치면 주기가 짧아도 사용자 요청이 만료를 만난다.
- 그래도 공백은 남으므로 **폴백을 함께 둔다**. 느리게 변하는 값은 stale-while-revalidate(만료값을 내리면서 뒤에서 갱신)로, 실시간 값은 stale 금지 + 조회불가로 — `ExternalDataCache.StalePolicy` 가 이 선택을 소유한다.
...
- **timeout 은 median 이 아니라 응답시간 분포의 꼬리에서 정한다.** 기준은 **p99(중요하면 p99.9)** 에 여유를 얹은 값이다. 표본 **최대값은 쓰지 않는다** — outlier 하나와 측정 횟수에 좌우돼 과하게 길거나 짧아진다. 분포 안쪽을 자르면 간헐 실패가 되고, 폴백이 조용히 화면 품질을 떨어뜨린다.
- **호출 하나의 timeout 과 작업 전체의 deadline 은 별개다.** 페이지·기간·후보를 순차로 도는 작업은 호출별 상한이 곱해지므로 **전체 상한을 따로 둔다.** 그리고 전체 상한이 **호출 하나까지 내려가야** 한다 — 남은 예산을 호출에 넘겨 짧은 쪽을 따르게 하지 않으면, 마감 직전에 시작한 마지막 호출이 자기 timeout 만큼 더 대기해 전체 상한이 그만큼 초과된다.
- 상한에 걸려 중단할 땐 **불완전한 결과를 버린다.** 부분 집계로 계산하면(뒷페이지가 0으로 과소집계 등) 계산을 포기하는 것보다 나쁘다 — 전자는 조용히 틀리고, 후자는 로그가 남는다.
...
- **분류·상태·타입은 enum.** boolean 플래그·문자열 코드 대신 enum 을 쓰고, 가능하면 **enum 에 행위를 담아**(상수별 메서드) 다형성으로 분기를 없앤다.
  - OffWay 예: `PolicyType`(혜택 분류 — 각 상수가 매칭조건·뱃지문구 보유), `TransportMode`(자가용·KTX·SRT·시외버스 — 상수별 소요시간 계산), `RegionType`.
...
- **혜택** — 여행자가 직접 신청해 받는 것. 지금 다루는 분류는 `PolicyType` 이 정본이다(숙박세일페스타·디지털관광주민증 등). **개수를 이름에 박지 않는다** — 분류도 실제 정책 건수도 백오피스에서 늘고 준다.
...
**스테이징은 파일을 하나씩 지정한다.** 작업 디렉터리에는 커밋 대상이 아닌 것이 늘 함께 있다 — 일일 리포트, 조사용 스크래치, 다른 작업의 미완성 변경.

실제로 그렇게 새어 나갔다. 기능 PR(기상청 날씨)에 일일 리포트 5개가 섞였고, 문서 PR 에 또 하나가 붙었다. 리뷰어는 봐야 할 것과 아닌 것을 매번 골라내야 했다.

```bash
git add src/main/java/com/offway/core/trip/domain/LicensedPlace.java   # 이렇게
git add -A                                                            # 이러지 않는다
```

`git status` 로 대상을 먼저 확인하고, 커밋 단위로 나눠 담는다. 여러 파일이면 여러 줄이 되는 것이 정상이다 — 그 수고가 곧 "이 커밋에 무엇이 들어가는가" 를 확인하는 절차다.

## 데이터셋을 늘리면 README 를 함께 고친다

**서비스가 어떤 데이터 위에 서 있는지는 README 의 "데이터 풀" 표가 정본이다.** 장소 풀에 소스를 더하거나(문화재·펜션 등) 분류를 바꾸면 그 표의 건수·출처·커버리지를 같은 PR 에서 갱신한다.

- 숫자는 **실측값**을 쓴다. 생성 스크립트가 찍는 출력이나 파일에서 직접 센 값이어야 한다 — 어림수는 금방 낡고, 낡은 숫자는 없는 것보다 나쁘다.
- 소스를 뺐을 때도 마찬가지다. 표에 남아 있으면 있는 줄 안다.
- 데이터가 늘어 **커버리지가 달라지면**(89곳 중 몇 곳) 그것도 함께 고친다. "전부 커버" 는 유지될 때만 사실이다.

이 표를 읽고 "이 서비스가 무엇을 알고 있는지" 를 판단한다. 코드보다 먼저 읽히는 자리라 어긋나면 오해가 그대로 굳는다.
