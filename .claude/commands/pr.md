...
- 70자 이내
- **커밋과 같은 형식 — `{타입}: {제목}`.** 타입은 `.githooks/commit-msg` 의 허용 목록을 따르고, 보통 이 PR 의 분류 라벨·브랜치 prefix 와 같다.
- 예: `feat: 공통 응답 래퍼·예외 체계·전역 예외 핸들러`, `chore: 개발 워크플로우 슬래시 커맨드 이식`
- update 모드에서는 기본적으로 제목 유지. 의도·스코프 변화나 정정이 필요한 경우만 변경하고 사용자에게 이유를 짚어 확인받는다.

> **왜 커밋과 같은 형식인가.** 사람이 올리는 PR 은 **squash 로 머지**한다. 커밋이 2개 이상인 PR 은 스쿼시 커밋 제목이 PR 제목으로 떨어지므로, PR 제목이 곧 `dev` 히스토리의 커밋 제목이 된다. 타입을 빼면 dev 에 타입 없는 커밋이 쌓여 커밋 컨벤션과 어긋난다.
>
> **dependabot PR 은 예외로 merge commit 으로 들어간다**(`dependabot-auto-merge.yml`). 봇 커밋을 그대로 남겨 어떤 버전에서 어떤 버전으로 갔는지가 히스토리에 보이게 하기 위해서다. 레포 설정상 merge commit 도 열려 있으므로, 사람이 올린 PR 을 머지할 때는 **squash 를 골랐는지 확인**한다.
>
