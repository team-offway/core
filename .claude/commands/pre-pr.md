...
**새 값을 더했으면 그 값을 만드는 자리와 내보내는 자리를 세어서 적는다.** 팩토리가 다섯이면 다섯을 다 봤는지, 응답이 넷이면 **넷을 다 보고 자리마다 포함·제외를 정했는지**. 숫자로 답한다.
...
세는 법은 `grep` 한 줄이다. 오래 걸리지 않는다. 아래는 **후보를 긁어모으는 예시**이지 전수 그 자체가 아니다 — 타입·접근자 이름을 이번 것으로 바꿔 쓴다.

```bash
# ① 이 타입을 만드는 자리 (팩토리·빌더 호출)
grep -rn "RegionAccess\.\|RegionAccessBuilder" src/main/java --include=*.java

# ② 이 값을 읽어 내보내는 자리 — 괄호를 붙이지 않는다
grep -rn "totalSlots\|getPoiContentId" src/main/java --include=*.java
```

**괄호를 붙여 검색하지 않는다.** `getPoiContentId()` 로 찾으면 `Slot::getPoiContentId` 같은 **메서드 참조가 빠진다** — 누락을 막자는 검색이 누락을 만든다. 생성 쪽도 마찬가지라 `new`·`toBuilder`·`with*` 같은 다른 표현이 남을 수 있다.

**결과가 0건인 것을 "누락 없음" 으로 읽지 않는다.** 이름을 안 바꿨거나 표현이 달라 안 걸린 경우가 대부분이다. 걸린 것을 눈으로 훑은 뒤 **실제 자리 수**를 적는다.
