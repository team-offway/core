...
- **DB 는 Testcontainers 로 띄운 MySQL** 이다(#175). 운영과 같은 DB 로 검증하려는 것이다 — H2 는 `MODE=MySQL` 이어도 문법·타입·제약이 달라, 마이그레이션이 로컬 초록인 채 MySQL 에서 깨진 적이 있다.
  - **도커가 필요하다.** 없으면 테스트가 안 돈다.
  - 컨테이너는 테스트 JVM 이 살아 있는 동안 **하나만** 뜬다(`jdbc:tc:` URL). 클래스마다 띄우면 부팅 비용이 클래스 수만큼 곱해진다.
  - **커넥션 풀을 작게 잡는다**(`maximum-pool-size=2`). stub 구성이 다르면 컨텍스트가 따로 만들어지고 실행 내내 캐시되는데, 컨텍스트마다 기본 10 커넥션을 잡으면 MySQL 상한(151)을 넘겨 `Too many connections` 가 난다. H2 에서는 없던 제약이다.
