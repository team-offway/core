...
tasks.withType<Test> {
    ...
    //
    // 이 스위트는 통합 테스트 상당수가 stub 구성이 달라 컨텍스트가 갈리고, Spring 은 그것들을 캐시해
    // 실행 내내 살려둔다(기본 상한 32개). 각 컨텍스트가 bean factory·Hibernate SessionFactory·
    // Hikari 풀을 따로 든다. **컨텍스트가 늘 때마다 이 값이 다시 문제가 된다.**
    //
    // 1차 실측(2026-08-10) — 그때는 1g 가 live set(557MB)의 1.8배라 넉넉했다.
    //
    //   512m  Old 피크 492MB (힙의 96%)  GC 417회  1m21s   ← CI 가 여기서 터졌다
    //   1g    Old 피크 559MB (55%)       GC  85회  1m02s
    //   2g    Old 피크 557MB (27%)       GC  74회  1m05s
    //
    // 2차 실측(2026-09-05, #433 에서 통합 테스트가 하나 더 늘며 CI 가 다시 OOM). **live set 이 그 사이
    // 두 배 가까이 커져 1g 가 한계에 닿았다.**
    //
    //   1g    Old **중앙** 985MB (96%)   GC 818회  2m28s   ← CI 가 여기서 터졌다
    //   2g    Old 중앙 292MB (14%)       GC  96회  1m23s
    //
    // 1g 의 중앙값이 985MB 라는 것은 **거의 내내 힙이 꽉 차 있었다**는 뜻이다. 818번의 GC 가 억지로
    // 눌러 담았고, 그 대가로 실행 시간이 두 배가 됐다. 2g 에서 중앙 292MB 로 떨어지는 것이 진짜 live
    // set 이고(피크 1,145MB 는 컨텍스트 여럿이 겹치는 순간이다), 1g 로는 그 피크를 담을 수 없다.
    //
    // 이번에는 2g 가 GC 를 줄이는 정도가 아니라 **실행 시간을 절반으로 되돌린다** — 1차 때 "2g 는 얻는
    // 게 없다" 고 적은 판단이 뒤집힌 자리다. 통합 테스트가 더 늘면 같은 방식으로 다시 재고 올린다.
    maxHeapSize = "2g"
